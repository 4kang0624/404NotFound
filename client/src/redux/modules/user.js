...
export const loginUser = (dataToSubmit) => {
  ...
  .then(res =>res.data).catch(err => alert(err.message));
  return {type: LOGIN, payload: request}
};

export const authorize = () => {
  const request = axios.post("/autoLogin")
...
export default function reducer(state = {}, action = {}){
  switch(action.type){
    ...
    case GET_USER_CONTENT:
      return {...state, getUserContentSuccess: action.payload.result, userContent: action.payload.userContent};  
    default: 
      return state;
  }
}
